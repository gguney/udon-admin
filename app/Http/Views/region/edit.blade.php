...
@section('content')
    <div class="w-form form-wrapper">
        <div class="w-form">
            {{ Form::model($region, array('route' => array('region.update', $region->id), 'method' => 'PUT')) }}
            <h3 class="heading">Edit a Region</h3>
            {!! csrf_field() !!}
            <label for="city" class="form-label">City</label>
            <select id="city" name="city" class="w-select select-field">
                @foreach($cities as $city)
                    @if($city->id == $region->ref_city_id)
            ...
            <input id="name" type="text" placeholder="Enter a name" name="name" data-name="name"
                   class="w-input text-field" value="{{ $region->name }}">
            @if ($errors->has('name'))
                <span class="help-block">
            ...
            <input id="postal_code" type="text" placeholder="Enter a postal code" name="postal_code" data-name="postal_code"
                   class="w-input text-field" value="{{ $region->postal_code }}">
            ...
            <input id="description" type="text" placeholder="Enter a description" name="description" data-name="description"
                   class="w-input text-field" value="{{ $region->description }}">
